...
function Product({id, name, pic, store, price, quantity, charity, description, donateList, setDonateList, setState}) {
  const card = {
    marginBottom: "20px",
    marginLeft: "15%",
    marginRight: "15%",
  ...
  function handleData(value, subtotal_value) {
    if (value > 0) {
      if (donateList.some(e => e.id === id)) { // 存在前一個相同id的資料
        let newDonateList = donateList.filter((e) => {
          return e.id !== id;
        })
        // console.log('newDonateList: ', newDonateList)
        newDonateList.push({id, name, pic, store, price, quantity, count: value, charity, description, subtotal: subtotal_value})
        setDonateList(newDonateList)
        localStorage.setItem("donateList", JSON.stringify(newDonateList));
      }
      else { // 不存在前一個相同id的資料
        donateList.push({id, name, pic, store, price, quantity, count: value, charity, description, subtotal: subtotal_value})
